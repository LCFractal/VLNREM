...
def select_edge(paths,ebcnode,shortest_distance,pos2d,pos3d):
    edge = {}
    for key in ebcnode.keys():
        ...
        edge[key] = {'count':0,
                     ...
                     'edgepos3d':[pos3d[key[0]],pos3d[key[1]]],
                     'edgevi': [vi1,vi2],
                     's_e':{'s':{},'m':{}},
                     'e_s':{'s':{},'m':{}}}
    def _add_edge(path,start,end,sub_path,sub_instruction,s2e=1):
        ...
        if sub_start == 1:
            ...
            sub_heading = np.arctan2(x, y) % (2 * np.pi)
...
def main(args):
  ...
  for scan, values in scans.items():
    ...
    all_edge = select_edge(values,ebcnode,shortest_distance,pos2d,pos3d)
...
def generate_RER2R(args):
    ...
    with open(os.path.join(args.connections_dir, 'scans.txt'), 'r') as f:
        outscans_lines = f.readlines()

    def _connections_file_path(scan):
        return os.path.join(
            args.connections_dir, '{}_connectivity.json'.format(scan))
    def _connections_outfile_path(scan):
        return os.path.join(
            args.connections_outdir, '{}_connectivity.json'.format(scan))
    ...
    for index in range(len(key1)):
        scan1 = key1[index]
        scan2 = key2[index]
        sub_scans1 = sub_scans[scan1][1]
        sub_scans2 = sub_scans[scan2][1]

        sub_tmp = {'count': 0}
        for sub_key1 in sub_scans1:
            sub_edge1 = sub_scans1[sub_key1]
            for sub_key2 in sub_scans2:
                sub_edge2 = sub_scans2[sub_key2]
                if sub_edge1['edgevi'] == sub_edge2['edgevi']:
                    if sub_tmp['count'] < sub_edge1['count'] * sub_edge2['count']:
                        sub_tmp = {
                            'count': sub_edge1['count'] * sub_edge2['count'],
                            's1': {
                                'count': sub_edge1['count'],
                                'edge': (sub_key1, sub_key1),
                                'first_edge': sub_edge1,
                                'second_edge': sub_edge1
                            },
                            's2': {
                                'count': sub_edge2['count'],
                                'edge': (sub_key2, sub_key2),
                                'first_edge': sub_edge2,
                                'second_edge': sub_edge2
                            }
                        }

        tmp[key1[index]+'+'+key2[index]] = sub_tmp
    sub_scans = tmp
    ...
    def _combine_path(path_set1,path_set2,edge1,edge2,pos2d1,pos2d2,scan1,scan2,vi1,vi2):
        ...
        p_heading1 = round(np.arctan2(x, y) % (2 * np.pi),3)
        x, y = pos2d2[1] - pos2d2[0]
        p_heading2 = round(np.arctan2(x, y) % (2 * np.pi),3)
        x, y = pos2d1[0] - pos2d1[1]
        se_heading1 = round(np.arctan2(x, y) % (2 * np.pi),3)
        x, y = pos2d2[0] - pos2d2[1]
        se_heading2 = round(np.arctan2(x, y) % (2 * np.pi),3)

        x, y = sub_graph[scan1+'+'+scan2]['pos2d'][edge2[1]] - sub_graph[scan1+'+'+scan2]['pos2d'][edge1[0]]
        rl_heading1 = round(np.arctan2(x, y) % (2 * np.pi), 3)
        vri1 = int(round(rl_heading1 * 6 / np.pi) % 12)

        x, y = sub_graph[scan1+'+'+scan2]['pos2d'][edge1[0]] - sub_graph[scan1+'+'+scan2]['pos2d'][edge2[1]]
        rl_heading2 = round(np.arctan2(x, y) % (2 * np.pi), 3)
        vri2 = int(round(rl_heading2 * 6 / np.pi) % 12)
    ...
    for index in range(len(key1)):
        ...
        scan1 = key1[index]
        scan2 = key2[index]
        if sub_scans[scan1+'+'+scan2]['count']==0:
            continue
        sub_scan1 = sub_scans[scan1+'+'+scan2]['s1']
        sub_scan2 = sub_scans[scan1+'+'+scan2]['s2']

        outscans_lines.append(scan1+'+'+scan2 + '\n')
        outscans_lines.append(scan2+'+'+scan1 + '\n')

        graph_utils.save_single(args.connections_outdir, _connections_file_path(scan1), _connections_file_path(scan2), scan1,
                         scan2, sub_scan1, sub_scan2, 'first_edge')
        graph_utils.save_single(args.connections_outdir, _connections_file_path(scan2), _connections_file_path(scan1), scan2,
                         scan1, sub_scan2, sub_scan1, 'first_edge')

        for key in [scan1+'+'+scan2, scan2+'+'+scan1]:
            tmpgraph = graph_utils.load(_connections_outfile_path(key))
            newpos2d = nx.get_node_attributes(tmpgraph, 'pos2d')
        ...
        for i_edge,x_edge in enumerate(['first_edge']):
            # scan1 to scan2
    ...
    for i in new_paths:
        if len_new_ins >= len_ins*PROPORTION:
            break
    ...
    if args.outscans:
        with open(os.path.join(args.connections_outdir,'scans.txt'),'w') as f:
            f.writelines(outscans_lines)
    if not os.path.isdir(os.path.dirname(args.output_file_path)):
