...
PLACE365_FEATURES = '../Recurrent-VLN-BERT/img_features/ResNet-152-places365.tsv'
...
def train(train_env, tok, n_iters, log_every=100, val_envs={}, aug_env=None):
    ...
    for idx in range(start_iter, start_iter+n_iters, log_every):
        ...
        rl_length = max(len(listner.logs['rl_loss']), 1)
        rl_loss = sum(listner.logs['rl_loss']) / rl_length
        ml_length = max(len(listner.logs['ml_loss']), 1)
        ml_loss = sum(listner.logs['ml_loss']) / ml_length
...
def train_val():
    ...
    val_env_names = ['train', 'val_unseen', 'val_seen']
