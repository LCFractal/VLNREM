...
class Seq2SeqAgent(BaseAgent):
    ...
    def get_input_feat(self, obs):
        ...
        input_a_t = torch.from_numpy(input_a_t).cuda() # 4个一循环，共128个
    ...
    def make_equiv_action(self, a_t, perm_obs, perm_idx=None, traj=None):
        ...
        for i, idx in enumerate(perm_idx):
            ...
            if action != -1:            # -1 is the <stop> action
                # todo: 从后选中选择一个目标，执行action到对应目标，traj记录action过程
    ...
    def rollout(self, train_ml=None, train_rl=True, reset=True, speaker=None):
        ...
        if train_rl:
            ...
            for t in range(length-1, -1, -1):
                ...
                critic_loss = (((r_ - v_) ** 2) * mask_).sum().item()
            ...
            self.loss += rl_loss
            self.logs['rl_loss'].append(rl_loss.cpu().detach().numpy())

        if train_ml is not None:
            self.loss += ml_loss * train_ml / batch_size
            self.logs['ml_loss'].append((ml_loss/batch_size).cpu().detach().numpy())
    ...
    def test(self, use_dropout=False, feedback='argmax', allow_cheat=False, iters=None):
        ...
        if use_dropout:
            ...
            self.encoder.eval() # words encoder
            self.decoder.eval()
            self.critic.eval()
