...
class Evaluation(object):
    ...
    def _get_nearest(self, scan, goal_id, path):        # 作用，查找路径中离终点最近的位置
        near_id = path[0][0]    # 起点ID
        near_d = self.distances[scan][near_id][goal_id] # 起点-终点距离
        for item in path:
            d = self.distances[scan][item[0]][goal_id]  # 路径每个位置到终点距离
            if d < near_d:                              # 如果小于起点到终点距离
                near_id = item[0]
                near_d = d
        return near_id
    ...
    def _score_item(self, instr_id, path):
        ...
        gt = self.gt[instr_id.split('_')[-2]]      # 获取gt
        start = gt['path'][0]
        assert start == path[0][0], 'Result trajectories should include the start position'
        goal = gt['path'][-1]
        final_position = path[-1][0]    # the first of [view_id, angle, vofv]
        nearest_position = self._get_nearest(gt['scan'], goal, path)                                # 路径中离终点最近位置
        self.scores['nav_errors'].append(self.distances[gt['scan']][final_position][goal])          # 最后位置到终点距离
        self.scores['oracle_errors'].append(self.distances[gt['scan']][nearest_position][goal])     # 最近点到终点距离
        self.scores['trajectory_steps'].append(len(path)-1)                                         # 路径步数
        distance = 0 # Work out the length of the path in meters
        prev = path[0]
        for curr in path[1:]:
            distance += self.distances[gt['scan']][prev[0]][curr[0]]
            prev = curr
        self.scores['trajectory_lengths'].append(distance)                                          # 路径长度
        self.scores['shortest_lengths'].append(                                                     # 到目标最短长度
    ...
    def score(self, output_file):
        ...
        score_summary = {
            'nav_error': np.average(self.scores['nav_errors']),         # 最后位置到终点距离
            'oracle_error': np.average(self.scores['oracle_errors']),   # 最近点到终点距离
            'steps': np.average(self.scores['trajectory_steps']),       # 路径步数
            'lengths': np.average(self.scores['trajectory_lengths'])    # 路径长度
        }
        num_successes = len([i for i in self.scores['nav_errors'] if i < self.error_margin])            # 小于阈值为成功
        score_summary['success_rate'] = float(num_successes)/float(len(self.scores['nav_errors']))      # 成功率
        oracle_successes = len([i for i in self.scores['oracle_errors'] if i < self.error_margin])      # oracle成功数
        score_summary['oracle_rate'] = float(oracle_successes)/float(len(self.scores['oracle_errors'])) # oracle成功率
